...
def test_delete_domain_certificate(bucket_name, domain, cert_name):
    ...
    path = f"/?domainCertificate&domain={domain}&certName={cert_name}"
...
if __name__ == "__main__":
    ...
    print(f"Testing delete domain certificate API with bucket name: {bucket_name}, domain: {domain}, cert name: {cert_name}")  
    print("--- 删除下行域名证书 ---")
    response = test_delete_domain_certificate(bucket_name, domain, cert_name)
    print(f"删除下行域名证书响应内容: {response}\n")
